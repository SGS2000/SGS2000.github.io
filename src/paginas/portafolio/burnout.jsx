    ...
    return (
        <>
            <div className="contenedorPrincipal">
                <h1>{es ? "Burnout en personal de salud" : "Occupational Burnout in Healthcare Workers"}</h1>
                <h2>{es ? "Introducción" : "Introduction"}</h2>
                <p>
                    {es ? "En este estudio, se evaluó el efecto del Síndrome de Desgaste Ocupacional (Burnout) en el personal de salud de una pequeña ciudad de Argentina durante la pandemia de COVID-19." : "This study analyzed the effects of Occupational Burnout Syndrome among healthcare workers in a small city in Argentina during the COVID-19 pandemic."}
                </p>
                <p>{es ? "Presentado como trabajo final de la materia Bioestadística (2021)." : "Presented as the final project for the Biostatistics course (2021)."} </p>
                {/* Metodologia */}
                <h2>{es ? "Metodología" : "Methodology"}</h2>
                <h3>{es ? "Diseño del estudio y recolección de datos" : "Study design and data collection"}</h3>
                <p>
                    {es ? "Se utilizó un estudio observacional de corte transversal. La población bajo estudio incluyó a todo trabajador empleado por algún hospital o clínica de la ciudad que haya tenido contacto con pacientes durante el período 2020-2021." : "A cross-sectional observational study was used. The population under study included any worker employed by any hospital or clinic in the city who had contact with patients during the 2020-2021 period."}
                </p>
                <p>
                    {es ? "Se utilizó un formulario online creado con la herramienta Google Forms, el cual fue enviado mediante e-mail a los distintos participantes. A los entrevistados se les preguntó por su sexo, edad, ocupación y número de convivientes. Para medir el efecto del burnout, se recurrió al Oldenburg Burnout Inventory (OLBI)" : "A Google Forms online survey was used for data collection, which was sent via email to the different participants. Respondents were asked about their gender, age, occupation, and number of cohabitants. To measure the effect of burnout, the Oldenburg Burnout Inventory (OLBI) was used."}
                </p>
                <h3>{es ? "Análisis estadístico" : "Statistical analysis"}</h3>
                <p>
                    {es ? "Se llevó a cabo un análisis descriptivo de las variables explicativas. Se calcularon medias, desvíos, cuartiles y rangos para las variables continuas; así como frecuencias absolutas y relativas para las variables discretas." : "A descriptive analysis of the explanatory variables was performed. Means, standard deviations, quartiles, and ranges were calculated for continuous variables, while absolute and relative frequencies were calculated for discrete variables."}
                </p>
                <p>
                    {es ? "Para evaluar el efecto de las covariables sobre el burnout se planteó un modelo de regresión logística, considerando como variable respuesta al score OLBI. Los resultados fueron interpretados mediante razones de odds." : "To evaluate the effect of the covariates on burnout, a logistic regression model was used, considering the OLBI score as the response variable. The results were interpreted using odds ratios."}
                </p>
                <p>
                    {es ? "Todos los cálculos fueron realizados con los programas Microsoft Excel y SAS." : "All calculations were performed with Microsoft Excel and SAS."}
                </p>
                {/* Resultados */}
                <h2>{es ? "Resultados" : "Results"}</h2>
                <h3>{es ? "Muestra" : "Sample"}</h3>
                <p>
                    {es ? "55 personas respondieron la encuesta, de las cuales 41 (74,5%) fueron mujeres. La edad media fue de 45,4 años, con un desvío estándar de 10,5. La mayoría de los participantes (69,1%) reportó vivir con dos personas o más, mientras que apenas 7 individuos (12,7%) vivían solos." : "Fifty-five people responded to the survey, of whom 41 (74.5%) were women. The mean age was 45.4 years, with a standard deviation of 10.5. The majority of participants (69.1%) reported living with two or more people, while only 7 individuals (12.7%) lived alone."}
                </p>
                <p>
                    {es ? "La muestra consistió de 23 médicos (41,8%), 12 enfermeros (21,8%), 12 administrativos (21,8%) y 8 profesionales de otra clase." : "The sample consisted of 23 physicians (41.8%), 12 nurses (21.8%), 12 administrative workers (21.8%), and 8 professionals from other fields."}
                </p>
