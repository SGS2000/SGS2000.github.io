    ...
    return (
        <>
            <div className="contenedorPrincipal">
                <h1>{es ? "Analizando datos de The Elder Scrolls III: Morrowind" : "Analyzing data from The Elder Scrolls III: Morrowind"}</h1>
                {/* Introduccion */}
                <h2>{es ? "Introducción" : "Introduction"}</h2>
                <p>
                    <i>The Elder Scrolls III: Morrowind </i>
                    {es ? " es un videojuego de rol (RPG) desarrollado por Bethesda Game Studios y lanzado en 2002. La historia principal tiene lugar en Vvardenfell, una isla en la provincia de Morrowind que forma parte del continente de Tamriel."
                ...
                <p>
                    {es ? "Para descargar el conjunto de datos, pulse " : "To download the dataset, click "}
                ...
                {/* Analisis SPSS */}
                <h2> {es ? "Estadística descriptiva (SPSS)" : "Descriptive Statistics (SPSS)"} </h2>
                <p>
                    {es ? "Los personajes del juego pueden pertenecer a una de diez razas. Entre ellas, la más común es " : "The NPCs belong to one of ten races. Among them, the most common is "}
                    <a href="https://en.uesp.net/wiki/Morrowind:Dark_Elf" target="_blank" rel="noreferrer"> Dark Elf / Dunmer</a>
                    {es ? " (más del 55% de los personajes), mientras que la menos frecuente es " : " (more than 55% of the characters), while the least frequent is "}
                ...
                <div className="contenedorGaleria">
                    <img src={es ? SPSSRaza1Es : SPSSRaza1En} alt="Razas" ></img>
                    <img src={es ? SPSSRaza2Es : SPSSRaza2En} alt="Razas sin Dark Elf"></img>
                </div>
                <p>
                    {es ? "Casi el 60% de los personajes son de sexo masculino. Esta diferencia es más marcada en las razas " : "Nearly 60% of the characters are male. This difference is more pronounced among the "}
                    ...
                    {es ? " (siendo más del 70% de los personajes de sexo masculino). Las únicas razas con más mujeres que hombres son " : " races (with over 70% of the characters being male). The only races with more women than men are "}
                ...
                <div className="contenedorGaleria">
                    <img src={es ? SPSSSexoEs : SPSSSexoEn} alt="Genero"></img>
                    <img src={es ? SPSSSexoTablaEs : SPSSSexoTablaEn} alt="Genero Tabla"></img>
                </div>

                <div className="contenedorGaleria">
                    <img src={es ? SPSSSexoRazaEs : SPSSSexoRazaEn} alt="Sexo y raza"></img>
                    <img src={es ? SPSSSexoRaza2Es : SPSSSexoRaza2En} alt="Sexo y raza sin Dark Elf"></img>
                </div>
                <p>
                    {es ? "Los personajes de Morrowind pueden pertenecer a una de de múltiples facciones, entre las cuales el jugador solo puede incorporarse a unas pocas. Para más detalles, consultar " : "The NPCs in Morrowind can belong to one of multiple factions, while only some of them are joinable by the player. For more details, refer to "}
                    <a href="https://en.uesp.net/wiki/Morrowind:Factions" target="_blank" rel="noreferrer">{es ? "este artículo" : "this article"}</a>.
                </p>
                <p>
                    {es ? "En casi todas las facciones hay una mayoría masculina, con la excepción del " : "In almost all factions there is a male majority, with the exception of the "}
                    <a href="https://en.uesp.net/wiki/Morrowind:Mages_Guild" target="_blank" rel="noreferrer">{es ? "Gremio de Magos" : "Mages Guild"}</a>
                    {es ? " (54,2% femenino) y los " : " (54.2% female) and the "}
                    <a href="https://en.uesp.net/wiki/Morrowind:Blades" target="_blank" rel="noreferrer">{es ? "Cuchillas" : "Blades"}</a>
                    {es ? " (igual cantidad de hombres que de mujeres). Respecto a las razas, en la mayoría de las facciones predominan los Dunmer, pero hay excepciones como por ejemplo la " : " (equal number of men and women). Regarding races, Dunmer are predominant in most factions, but there are exceptions such as the "}
                    <a href="https://en.uesp.net/wiki/Morrowind:Imperial_Legion" target="_blank" rel="noreferrer">{es ? "Legión Imperial" : "Imperial Legion"}</a>.
                </p>

                <div className="contenedorGaleria">
                    <img src={es ? SPSSFaccionEs : SPSSFaccionEn} alt="Faccion" className="imgGrande"></img>
                    <img src={es ? SPSSFaccionSexoEs : SPSSFaccionSexoEn} alt="Faccion y sexo"></img>
                    <img src={es ? SPSSFaccionRazaEs : SPSSFaccionRazaEn} alt="Faccion y raza"></img>
                </div>

                {/* Machine learning R */}
                <h2> {es ? "Aprendizaje automático (R)" : "Machine learning (R)"} </h2>
                <h3> {es ? "Habilidades en Morrowind" : "Skills in Morrowind"} </h3>
                <p>
                    {es ? "Hay 27 habilidades en Morrowind, que determinan qué tan bien se pueden realizar diversas tareas. Las habilidades se dividen dentro de tres especializaciones: combate, magia y sigilo. Para más información sobre las habilidades, revisar " : "There are 27 skills in Morrowind, each of which determines how well various tasks can be performed. The skills are divided into three specializations: combat, magic, and stealth. For more information about the skills, please refer to "}
                    <a href="https://en.uesp.net/wiki/Morrowind:Skills" target="_blank" rel="noreferrer">{es ? "este artículo" : "this article"}</a>.
                </p>
                <p>
                    {es ? "A continuación, se muestran las distribuciones de las distintas habilidades, según especialidad:" : "The following plots show the distributions of the different skills, displayed according to specialization:"}
                </p>

                <div className="contenedorGaleria">
                    <img src={es ? RSkillsCEs : RSkillsCEn} alt="Habilidades combate" className="imgGrande"></img>
                    <img src={es ? RSkillsMEs : RSkillsMEn} alt="Habilidades magia" className="imgGrande"></img>
                    <img src={es ? RSkillsSEs : RSkillsSEn} alt="Habilidades sigilo" className="imgGrande"></img>
                </div>

                <p>
                    {es ? "Se calculan las correlaciones entre habilidades. Como era de esperarse, las habilidades de una misma especialidad muestran las mayores correlaciones entre sí. La correlación positiva más fuerte se da entre Alteración y Misticismo (0,88), mientras que la correlación negativa más fuerte se da entre Atletismo y Misticismo (-0,55)" : "Correlations between skills are calculated. As expected, skills within the same specialization show the highest correlations. The strongest positive correlation is observed between Alteration and Mysticism (0.88), while the strongest negative correlation is between Athletics and Mysticism (-0.55)."}
                </p>
                <div className="contenedorImagenCentrada">
                    <img src={RMatriz} alt="Matriz habilidades" className="imgCentradaGrande"></img>
                </div>
                <h3> {es ? "Clasificación supervisada" : "Supervised learning"} </h3>
                <p>
                    {es ? "Las distintas facciones prefieren diferentes habilidades, cómo se muestra en la siguiente tabla: " : "Various factions favor different skills, as shown in the following table:"}
                </p>
                <div className="contenedorImagenCentrada">
                    <img src={RTabla} alt="Tabla habilidades facciones" className="imgCentradaGrande"></img>
                </div>
                <p>
                    {es ? "Se plantea como objetivo clasificar los personajes dentro de su facción correspondiente, utilizando sus niveles de habilidad respectivos como variables explicativas. Las técnicas que se contrastan son:" : "The objective is to classify characters into their respective factions, using their skill levels as explanatory variables. The techniques compared are :"}
                </p>
                <ul>
                    <li> {es ? "Árboles de decisión" : "Decision Trees"} </li>
                    <li> {es ? "Bosques aleatorios" : "Random Forest"} </li>
                    <li> {es ? "Clasificador bayesiano ingenuo" : "Naive Bayes classifier"} </li>
                    <li> {es ? "K vecinos más cercanos" : "K-nearest neighbors"} </li>
                </ul>
                <p>
                    {es ? "El mejor desempeño se obtuvo con bosques aleatorios. Los resultados completos son los siguientes: " : "The best performance was achieved with random forests. The complete results are as follows:"}
                </p>
                <div className="contenedorImagenCentrada">
                    <img src={es ? RResEs : RResEn} alt="Tabla resultados" className="imgCentradaGrande"></img>
                </div>

                <p>
                    {es ? "Analizando los resultados obtenidos con bosques aleatorios, se observa que las facciones con mayor precisión por clase fueron el " : "Analyzing the results obtained with random forests, it can be observed that the factions with the highest class accuracy were the "}
                    <a href="https://en.uesp.net/wiki/Morrowind:Tribunal_Temple" target="_blank" rel="noreferrer">{es ? "Templo del Tribunal" : "Tribunal Temple"}</a>
                    {es ? " y la " : " and "}
                    <a href="https://en.uesp.net/wiki/Morrowind:House_Hlaalu" target="_blank" rel="noreferrer">{es ? "Casa Hlaalu" : "House Hlaalu"}</a>.
                    {es ? " Las facciones con menor precisión por clase fueron el " : " The factions with the lowest class accuracy were the "}
                    <a href="https://en.uesp.net/wiki/Morrowind:Fighters_Guild" target="_blank" rel="noreferrer">{es ? "Gremio de Luchadores" : "Fighters Guild"}</a>
                    {es ? " y el " : " and the "}
                    <a href="https://en.uesp.net/wiki/Morrowind:Imperial_Cult" target="_blank" rel="noreferrer">{es ? "Culto Imperial" : "Imperial Cult"}</a>.
                    {es ? ' Las habilidades que presentaron una mayor importancia para el algoritmo fueron  "destruction", "short blade" y "light armor", mientras que "spear" y "conjuration" fueron las que presentaron una menor importancia.  ' : ' The skills that showed the greatest importance for the algorithm were "destruction," "short blade," and "light armor", while "spear" and "conjuration" were the least important. '}
                </p>

                <div className="contenedorGaleria">
                    <img src={es ? RConfusionEs : RConfusionEn} alt="Confusion" className="imgGrande"></img>
                    <img src={es ? RImpEs : RImpEn} alt="Importancia" className="imgGrande"></img>
                </div>

                {/* Análisis Multivariado SAS */}
                <h2> {es ? "Análisis multivariado (SAS)" : "Multivariate statistics (SAS)"} </h2>
                <h3> {es ? "Análisis de componentes principales" : "Principal component analysis"} </h3>
                <p>
                    {es ? "Se realiza un análisis de componentes principales utilizando las habilidades mostradas anteriormente como variables. Se utilizan dos componentes principales: la primera distingue entre aquellos personajes especializados en magia y aquellos especializados en combate, mientras que la segunda distingue entre los personajes que están especializados en sigilo de los que no lo están. A continuación se presenta el gráfico de cargas:"
                ...
                <p>
                    {es ? "De manera similar, se construyen gráficos de scores para las distintas razas." : "Similarly, score plots are constructed for the different races."}
                </p>
                <div className="contenedorImagen">
                ...
                <h3> {es ? "Análisis de correspondencias" : "Correspondence analysis"} </h3>
                <p>
                    {es ? "Se aplica el análisis de correspondencias para estudiar la relación entre las razas de los personajes de Morrowind y la facción a la que pertenecen. La siguiente tabla de contingencia muestra todas las razas del juego y las diez facciones principales: "
